...
// ExtractDataFromImageWithOptions 调用 Qwen API 提取图片数据（扩展版，支持直接使用 URL）
// useDirectUrl: 是否直接使用 URL，不进行 Base64 转换
func (c *QwenClient) ExtractDataFromImageWithOptions(fileHeader *multipart.FileHeader, imageURL string, officialName string, appType string, useDirectUrl bool) (*model.ExtractedData, error) {
	startTime := time.Now()

	// 记录输入来源
	if fileHeader != nil {
		log.Printf("Qwen开始处理图片 - 来源: 文件上传, 大小: %d bytes, 姓名: %s, 类型: %s, 直接使用URL: %v",
			fileHeader.Size, officialName, appType, useDirectUrl)
	} else if imageURL != "" {
		log.Printf("Qwen开始处理图片 - 来源: URL下载, URL: %s, 姓名: %s, 类型: %s, 直接使用URL: %v",
